...
pacman::p_load(tidyverse, mgcv, patchwork, spdep)
```
...
data <- data_pre |> 
  ...
  mutate(prop.to.baseline = N_19F / (N_19Fbaseline + 0.05),
         prop.prop.to.baseline = (N_19F / (N_IPD + 0.05)) / ((N_19Fbaseline + 0.05) / (N_IPDbaseline + 0.05)))
```
...
```{r}
grid_df <- expand.grid(
  CAL_YR = unique(data_mrf$CAL_YR),
  agegrouporder = seq_along(unique(data_mrf$agegroup5yr))
)
...
grid_sf <- grid_df |> 
  rowwise() |> 
  mutate(geometry = st_sfc(make_square(CAL_YR, agegrouporder))) |> 
  ungroup() |> 
  st_as_sf() |> 
  arrange(CAL_YR, agegrouporder) |> 
  mutate(region.id = factor(row_number()))

nb <- poly2nb(grid_sf, queen = FALSE)
names(nb) <- grid_sf$region.id

bym_mod_df <- data_mrf |> 
  left_join(grid_sf |> st_drop_geometry(), by = c("CAL_YR", "agegrouporder")) |> 
  ungroup()

gam_mrf <- gam(prop.19F ~ s(region.id, bs = "mrf", xt = list(nb = nb)) +
                s(region.id, bs = "re"),
               data = bym_mod_df, 
               method = "REML")
...
pred_grid <- grid_sf |>
  st_drop_geometry() |>
  left_join(data_mrf |> distinct(agegroup5yr, agegrouporder), 
            by = "agegrouporder")

pred_grid$fit <- predict(gam_mrf, newdata = pred_grid, type = "response")
```
...
pred_grid |>
  ...
  labs(
    title = "Prop 19F (mrf+re smoothed)",
    x = "Year",
    y = "Age Group",
    fill = "19F prop (smoothed)"
  ) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
